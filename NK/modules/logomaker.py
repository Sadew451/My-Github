...
async def lego(event):
 ...
 try:
    ...
    img.save(fname, "png")
    await tbot.send_file(event.chat_id, file=fname, caption="Made BY @AnimX_Robot")         
    await pesan.delete()
    if os.path.exists(fname):
            os.remove(fname)
 except Exception as e:
    await event.reply(f'Error, Report @senuinfinity, {e}')
...
async def lego(event):
 ...
 try:
    ...
    img.save(fname2, "png")
    await tbot.send_file(event.chat_id, fname2, caption="Made BY @AnimX_Robot")
