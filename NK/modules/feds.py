...
import NK.modules.sql.feds_sql as sql
from NK import DRAGONS, EVENT_LOGS, LOGGER, OWNER_ID, TIGERS, WOLVES, dispatcher
...
def new_fed(update: Update, context: CallbackContext):
    ...
    if not fednam == "":
        ...
        if not x:
            update.effective_message.reply_text(
                "Can't federate! Please contact @kavinduaj if the problem persist."
            )
            return
...
def join_fed(update: Update, context: CallbackContext):
    ...
    if len(args) >= 1:
        ...
        if not x:
            message.reply_text(
                "Failed to join federation! Please contact @kavinduaj should this problem persist!"
            )
            return
...
def fed_ban(update: Update, context: CallbackContext):
    ...
    if user_id in [1548967589, 1548967589]:
...
def set_frules(update: Update, context: CallbackContext):
    ...
    if len(args) >= 1:
        ...
        if not x:
            update.effective_message.reply_text(
                "Whoa! There was an error while setting federation rules! If you wondered why please ask it in @kavinduaj !"
            )
            return
    ...
    if args:
        if args[0] == "json":
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "mrjoker_fbanned_users.json"
                update.effective_message.reply_document(
                    document=output,
                    filename="mrjoker_fbanned_users.json",
